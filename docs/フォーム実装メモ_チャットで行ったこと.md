...
- **Cursor で編集 → 本番反映の流れ（別チャット用）** … `docs/本番アップロードの流れ.md`

---

## 7. Chatwork で「自分に通知が届く」ようにする（別アカウント方式）

**状況:** API トークンは「そのトークンを持っているアカウント」として投稿される。自分のアカウントのトークンを使うと「自分が投稿した」扱いになり、自分宛ての通知が来ない。

**対応（推奨）:** **通知用の別アカウント**（例: アネラカフェ 川崎駅前店）を作り、そのアカウントの API トークンで投稿する。

- そのアカウントがルームに投稿すると、**自分（個人アカウント）には「他者からの投稿」として通知が届く**。
- 運用としては「LP からの問い合わせはこのアカウントが投稿する」と分かりやすく、他に良い代替もほぼない。

**手順のイメージ**

1. 新しい Chatwork アカウントを作成（別メールで「アネラカフェ 川崎駅前店」などで登録）。
2. そのアカウントで Chatwork にログイン → **設定** → **API** で API トークンを発行。
3. 通知を受け取りたいルームに、**その新しいアカウントをメンバーとして追加**（ルームのメンバー設定から招待）。
4. LP 側（Vercel の環境変数）の **`CHATWORK_API_TOKEN`** を、**新しいアカウントの API トークン** に差し替える。`CHATWORK_ROOM_ID` はそのままでよい。
5. これで問い合わせ時に「アネラカフェ 川崎駅前店」から投稿され、自分には通知が届く。

**注意:** トークンは「誰として投稿するか」だけを変えるもので、ルーム ID は変えません。ルームは同じまま、投稿者だけが「別アカウント」になります。
